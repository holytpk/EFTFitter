...
int main(/*int argc, char** argv*/) {
  ...
  const std::string sufDat = "_shape_data_a", sufSim = "_shape_part_cut_0";
  ...
  // Global vectors
  std::vector<std::string> v_hStr = {
      /*"b1k", "b2k",*/ "b1r", "b2r", "b1n", "b2n", "b1j", "b2j",
      "b1q", "b2q", "ckk", "crr", "cnn", 
      "c_Prk", "c_Mrk", "c_Pnr", "c_Mnr", "c_Pnk", "c_Mnk", 
      "c_kj", "c_rq", "c_han", "c_tra", "c_sca",
      "c_Prj", "c_Mrj", "c_kjL", "c_rqL", "c_rkP", "c_rkM", "c_nrP", "c_nrM", "c_nkP", "c_nkM", 
      "cHel", "cLab"/*, "ll_dPhi", "ll_deta"*/ 
  };
  
  std::vector<std::array<int, 2>> v_snake = {
    /*{1, 6}, {7, 12},*/ {13, 18}, {19, 24}, {25, 30}, {31, 36},
    {37, 42}, {43, 48}, {49, 54}, {55, 60}, {61, 66}, {67, 72},
    {73, 78}, {79, 84}, {85, 90}, {91, 96}, {97, 102}, {103, 108},
    {109, 114}, {115, 120}, {121, 126}, {127, 132}, {133, 138},
    {139, 144}, {145, 150}, {151, 156}, {157, 162}, {163, 168},
    {169, 174}, {175, 180}, {181, 186}, {187, 192}, {193, 198},
    {199, 204}, {205, 210}, {211, 216}/*, {217, 222}, {223, 228}*/
  };
  ...
  for (int iVar = 0; iVar < v_hStr.size(); ++iVar) {
    ...
    for (int iFit = 0; iFit < v_hStr.size(); ++iFit) {
      ...
      auto f_emulate = [&v_iEB, &rate_data, &rate_zero] (const auto &v_binC) {
        ...
        const double shapeSum = 38.;
      
        const std::array<double, 2> rate_mc = {v_binC.at(0).at(0) / shapeSum, rate_data.at(1)};
        std::vector<std::array<double, 2>> v_tmpC = {
          (rate_data.at(0) > 0.) ? rate_mc : rate_zero, rate_zero
        };
        ...
        // // ❗Only print once using static flag
        // static bool printed = false;
        // if (!printed) {
        //   std::cout << "DEBUG f_emulate: kept bins:\n";
        //   for (size_t i = 0; i < v_tmpC.size(); ++i)
        //     std::cout << "  [" << i << "] = {" << v_tmpC[i][0] << ", " << v_tmpC[i][1] << "}\n";
        //   printed = true;
        // }
      ...
      //                 dName + "snake_spinCorr" + sufSim, sName, 1, {34. * k_nnlo_nlo * 681.63, 0.}, EFT::Stat::count);

      // // and EFT
      // eft.addRawInput("ctG_2", EFT::Sample::all, inDir + "nlo_ctG_p2_nominal_toppt_default_shape.root", 
      //                 dName + "snake_spinCorr" + sufSim, sName, 1, {34. * k_nnlo_nlo * 1260.07, 0.}, EFT::Stat::count);

      // eft.addRawInput("ctG_-2", EFT::Sample::all, inDir + "nlo_ctG_m2_nominal_toppt_default_shape.root", 
      //                 dName + "snake_spinCorr" + sufSim, sName, 1, {34. * k_nnlo_nlo * 400.97, 0.}, EFT::Stat::count);

      // eft.addRawInput("ctG_0", EFT::Sample::all, "./concatenated_histograms_ctg0.root", 
      //                 "diff_cross_section", "", 1, {34. * k_nnlo_nlo * 681.63, 0.}, EFT::Stat::count);

      // // and EFT
      // eft.addRawInput("ctG_2", EFT::Sample::all, "./concatenated_histograms_ctg2.root", 
      //                 "diff_cross_section", "", 1, {34. * k_nnlo_nlo * 1260.07, 0.}, EFT::Stat::count);

      // eft.addRawInput("ctG_-2", EFT::Sample::all, "./concatenated_histograms_ctg-2.root", 
      //                 "diff_cross_section", "", 1, {34. * k_nnlo_nlo * 400.97, 0.}, EFT::Stat::count);

      eft.addRawInput("ctG_0", EFT::Sample::all, "./concatenated_histograms_ctg0.root",
                    "diff_cross_section", "", 1, fake_rate, EFT::Stat::count);
      ...
      eft.addRawInput("ctG_4", EFT::Sample::all, "./concatenated_histograms_ctg4.root",
                    "diff_cross_section", "", 1, fake_rate, EFT::Stat::count);
    
      eft.addRawInput("ctG_-4", EFT::Sample::all, "./concatenated_histograms_ctg-4.root",
                    "diff_cross_section", "", 1, fake_rate, EFT::Stat::count);         

      eft.addRawInput("ctG_8", EFT::Sample::all, "./concatenated_histograms_ctg8.root",
                    "diff_cross_section", "", 1, fake_rate, EFT::Stat::count);
    
      eft.addRawInput("ctG_-8", EFT::Sample::all, "./concatenated_histograms_ctg-8.root",
                    "diff_cross_section", "", 1, fake_rate, EFT::Stat::count);            
      ...
      if (useData) {
        auto f_shape_data = [&v_iEB, &rate_zero] (const auto &v_binC) {
          ...
          // static bool printed = false;
          // if (!printed) {
          //   std::cout << "DEBUG f_shape_data: kept bins:\n";
          //   for (size_t i = 0; i < v_tmpC.size(); ++i)
          //     std::cout << "  [" << i << "] = {" << v_tmpC[i][0] << ", " << v_tmpC[i][1] << "}\n";
          //   printed = true;
          // }
        ...
        eft.addHybridData("./concatenated_histograms.root", "diff_cross_section", 
      ...
      eft.readCovMatRoot("finalcov", "./covariance_matrix/final_covariance_matrix.root", "covariance_matrix", covMat_binRange);

      eft.listKeyToFit({ {"ctG", v_opPoint} });
      eft.computeFitChi2(v_sample);

      eft.draw1DChi2({ {"ctG", { "ctG", {-5., 5.}, {0., 9.999}, {-5., 5.} }} }, outDir, v_sample);
