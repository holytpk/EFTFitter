...
int main() {
  ...
  const std::string inDir = "./example_fit_input/";

  // dummy rate as usual
  const std::array<double, 2> rate_zero = {0., 0.};
  const std::array<double, 2> rate_data = {836.925, 51.685};
  ...
  const std::vector<std::string> v_operator = {"ut", "cvv"};
  ...
  */
  ...
  for (uint iOp1 = 0; iOp1 < v_operator.size() - 1; ++iOp1) {
    for (uint iOp2 = iOp1 + 1; iOp2 < v_operator.size(); ++iOp2) {
      ...
      // std::vector<std::string> key = {op1N, op2N};
      // if (m_range_var.find(key) == m_range_var.end()) {
      //   std::cerr << "Key not found: {" << op1N << ", " << op2N << "}" << std::endl;
      //   std::cerr << "Available keys:" << std::endl;
      //   for (const auto &p : m_range_var) {
      //     std::cerr << "  {" << p.first.at(0) << ", " << p.first.at(1) << "}" << std::endl;
      //   }
      //   exit(1);
      // }  
      // const double op1R = m_range_var.at({op1N, op2N}).first.at(0);
      // const double op2R = m_range_var.at({op1N, op2N}).first.at(1);
      // const std::vector<int> &v_variable = m_range_var.at({op1N, op2N}).second;        
      const int binToIgnore = 2;               

      const std::string op0S = v_syst.at(0), op1S = v_syst.at(1), op2S = v_syst.at(2);

      const std::string outDir = "./example_fit_output/shape/sm_" + op0S + "_" + op1N + "_" + op1S + "_" + op2N + "_" + op2S + "/";          
      ...
      auto f_emulate = [&v_variable, &rate_zero, &binToIgnore, &v_spinvar] (const auto &v_binC) {
        ...
        //TF1 *f_ref;
        TF1 *f_ref = nullptr;  // initialize to nullptr       
      ...
      std::cout<<"Ling: checking if histogram input is ready" << std::endl; 
          
      //eft.assignAsData("data", EFT::Sample::all, false);

      std::cout << inDir + op1N + "_coeff.root" << std::endl; 
      ...
      std::cout<<"Ling: checking if covmat input is ready" << std::endl; 
